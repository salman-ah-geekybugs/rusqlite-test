...
fn main() -> Result<()> {
    let repo = ClipboardRepository{
        database:String::from("test.db")
    };
    repo.init();
    let me = Clipboard {
        id: 0,
        message: "David".to_string(),
        current_window: "sublime text".to_string(),
        process: "Sublime".to_string()
    };

    repo.add(me);

    let clipboard_iter = repo.find_all();
    println!("{:?}",clipboard_iter );
    Ok(())
}